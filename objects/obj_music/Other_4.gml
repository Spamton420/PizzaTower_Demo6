if ((global.panic == 0) && (global.snickchallenge == 0))
{
    ...
    if ((string_letters(roomname) == "hubroom") || ((string_letters(roomname) == "Titlescreen") || ((string_letters(roomname) == "hub_entrancehall") || (string_letters(roomname) == "halloweenentrance"))))
    {
    ...
    {
        if (!audio_is_playing(mu_entrancesecret))
        {
            audio_stop_all()
            scr_sound(mu_entrancesecret)
            audio_sound_set_track_position(global.music, fadeoff)
            pausedmusic = mu_entrancesecret
        }
    }
